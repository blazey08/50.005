            ...
            while (!connectionSocket.isClosed()) {
                ...
                if (packetType == -1) {
                    ...
                    try {
                        ...
                        fileIO.receiveEncryptedFile(fromClient, privateServerKey);
                    ...
                    c.init(Cipher.DECRYPT_MODE, privateServerKey);
                    ...
                    Cipher ls = Cipher.getInstance("RSA/ECB/PKCS1Padding");
                    ls.init(Cipher.ENCRYPT_MODE, privateServerKey);
                    for (String s : f) {
                    ...
                    try {
                        fileIO.sendEncryptedFile(toClient, fileToSend, privateServerKey);
                    } catch (Exception e) {
                        toClient.writeInt(-1);
                    }
...
class fileIO {
    ...
    public static void receiveEncryptedFile(DataInputStream fromClient, Key key) throws Exception {
    ...
    public static void sendEncryptedFile(DataOutputStream toServer, String fileToSend, Key key) throws Exception {
        FileInputStream fileInputStream = new FileInputStream(fileToSend);
        BufferedInputStream bufferedFileInputStream = new BufferedInputStream(fileInputStream);
        Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
        ...
        while ((count = bufferedFileInputStream.read(Buffer)) > 0) {
            ...
            toServer.write(encrytedBuffer, 0, 128);
